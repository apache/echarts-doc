...
当 `dataMin` 小于实际数据最小值时，会使用一个不大于 `dataMin` 的整齐刻度值作为坐标轴最小值。
...
当 `dataMax` 大于实际数据最大值时，会使用一个不小于 `dataMax` 的整齐刻度值作为坐标轴最大值。
...
yAxis: {
    type: 'value',
    dataMax: 100  // 确保 Y 轴包含 100，但仍使用整齐刻度
}
```
